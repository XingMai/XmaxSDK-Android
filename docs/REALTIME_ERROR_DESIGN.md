...
| disconnect/close 的清理失败 | 日志；继续释放其他资源 | 无 | 只有可安全复用的资源 |
...
监听替换或显式传 `null` 会使尚未派发的旧注册失效；disconnect 和 close 均保留公共监听注册。已经开始执行的用户回调无法撤回。监听器自身抛异常仅记日志，不再次触发 fatal，不影响清理。
...
XLab 已实现这个接入方式；每轮生成会更新统一错误监听，页面销毁时显式注销，源切换或后台恢复调用 close 不依赖自动注销。参考图上传复用实时页面的 XmaxClient，避免第二个客户端改变全局日志选项。
